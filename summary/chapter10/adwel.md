...
## 분열, 공황, 그리고 혼란
- 마이크로 서비스 모니터링 환경은 더 큰그림을 볼 수 있도록 집계를 제공해야 함
- 운영 환경테서 테스트한다는 개념을 수용해 시스템 정상 상태에 대하 더 현명한 사고 방식을 가져야 함

## 단일 마이크로서비스, 단일 서버
- 매우 간단한 구성, 호스트가 하나이므로 인스턴스 자체에 접근하여 모니터링 해도 어려움이 없음

## 단일 마이크로서비스, 다수 서버
- 여로 호스트의 메트릭, 로그를 집계하고 분석해 줄 도구가 필요
- 로드 밸런서의 모니터링 및 서비스에서 장애가 났을 시 적절한 트래픽 분배가 필요

## 다수 마이크로서비스, 다수 서버
- 매트릭과 로그 뿐만 아닌 유입되는 데이터를 선별하고 추적해야 함
- 관찰가능성과 운영환경에서의 테스트로 사고방식을 전환
...
- 관찰 가능성은 시스템의 속성이며, metric, event, log, trace 와 같은 개념에 묶여서는 안됨
- 시스템을 관찰 가능하게 만들려고 할 때는 수집하고 조사할 수 있는 이벤트 측면에서 필요한 출력을 생각해야 함
...
- 로그 집계는 마이크로 서비스를 구현하기 위한 전제조건, 그만 큼 유용하고 중요

#### 공통 포맷
- 합리적인 표준 로그 형식을 만들어 기록해야 함
- 로그에서 필요한 정보를 구문 분석하고 추출하도록 집계 도구를 구성해야 함

#### 로그 문자열의 상관관계
- 마이크로 서비스에서 발생하는 모든 활동에 관한 로깅은 동일한 상관관계 ID로 설정해야 함
- 상호작용이 발생하는 트랜잭션을 추적하고 식별하는데 유용함
...
#### 단점
- 시간 왜곡으로 인해 호출이 발생한 순서를 이해하기 어려울 수 있음
- 호출이 많아 질 수록 로그 데이터는 더 많아지며 비용문제로 이어짐
- 로그를 집계 솔루션을 구축하고 유지하는 비용이 발생
- 민감한 데이터에 대한 접근을 제한해야 함
...
- 높은 카디널리티를 처리하는 시스템은 좀 더 다양한 정보를 제공해 줄 수 있음

### 분산 추적
- 시스템이 복잡해짐에 상관관계의 흐름을 보는 방법이 중요

#### 작동 원리
- 로컬활동은 span 로 캡처되고 고유 식별자로 연결되어 trace로 구성
- 샘플링을 사용하여 일부 trace만 수집하여 영향도를 최소화 해야 함

#### 분산 추적 구현
- 어플리케이션 내에서 수집 -> 수집기로 전송 -> 시각화 도구

### 잘하고 있나요?
- 마이크로 서비스에서 관찰 가능성 구축이 과도하게 수행하고 있는지, 아니면 충분하지 못하지 판단하기 어려움
- 필요한 리소스, 허용 가능한 응답 시간을 만드는 요소가 무엇인지을 결정하여 서비스가 정상인지 식별
...
- 특정 프로세스의 응답 시간, 등록 중인 고객, 고객에게 제기도니 오류 또는 진행 중인 주문이 될 수 있음
...
- 어떤 종류의 문제를 알리고 어떻게 알려하는지 정확하게 파악 해야 함
